...
@validate_staff_token()
def get_subscribed_lives(request, token, offset=0, limit=10, order=1):
    """获取自己预约的直播间
    :param token: 令牌(必传)
    :param offset: 起始值
    :param limit: 偏移量
...
@validate_staff_token()
def subscribe_live(request, token, live_id):
    """预约直播间
...
@validate_staff_token()
def get_playbacks(request, token, live_id, page_index=1, page_num=10):
    """
